...
```{r, section 1, chunk 6}
cnv <- cnv_clean(penncnv = cnv_penn_ars, #when running your own data, set this argument to 'penncnv = "localpath/file"'
...
```{r, chunk 7}
cnv <- cnv_clean(cnvpartition = cnv_part_umd, #when running your own data, set this argument by 'cnvpartition = "localpath/file"' 
                 drop_length = 5, #the maximum CNV length threshold (in Mb); CNVs larger than this value will be deleted
                 folder = "cnv_clean") 
```
...
```{r, chunk 9}
cnv <- cnv_clean(standard_cnv = cnv_other_umd, #when running your own data, set this argument by 'standard_cnv = "localpath/file"'
                 drop_length = 5, #the maximum CNV length threshold (in Mb); CNVs larger than this value will be deleted
                 folder = "cnv_clean")
```
...
```{r, section 2.4}
dir.create("call_gene_Penn_UCSC")
download.file("https://raw.githubusercontent.com/JH-Zhou/HandyCNV/master/vignettes/call_gene_Penn_UCSC/cnv_annotation.txt",destfile = "call_gene_Penn_UCSC/cnv_annotation.txt")
...
download.file("https://raw.githubusercontent.com/JH-Zhou/HandyCNV/master/vignettes/refgene/cow_ARS_UCSC.txt",destfile = "refgene/cow_ARS_UCSC.txt")
...
cnvr <- call_cnvr(clean_cnv = "cnv_clean/penncnv_clean.cnv", #standard file was generated by 'cnv_clean' function in section 1
...
```{r, section 5.2, chunk 22}
download.file("https://raw.githubusercontent.com/JH-Zhou/HandyCNV/master/vignettes/refgene/cow_ARS_ENS.txt",destfile = "refgene/cow_ARS_ENS.txt")
...
Figure.11 All high frequency CNVRs from PennCNV

high_freq_cnvr_penn/combine_cnvr.png{width=320px}
...
```
Figure.12 Comparison of PennCNV and CNVPartition results

compare_cnv_PartUMD_Vs_PennUMD/combine.png{width=330px}
...
```
Figure.13 Comparison of CNVR

compare_cnvr_PartUMD_Penn_UMD/combine_cnvr.png {width=320px}
...
download.file("https://raw.githubusercontent.com/JH-Zhou/HandyCNV/master/vignettes/Demo_data/demo_intensity.txt",destfile = "Demo_data/demo_intensity.txt")

download.file("https://raw.githubusercontent.com/JH-Zhou/HandyCNV/master/vignettes/Demo_data/demo.map",destfile = "Demo_data/demo.map")

download.file("https://raw.githubusercontent.com/JH-Zhou/HandyCNV/master/vignettes/Demo_data/demo_HandyCNV.bed",destfile = "Demo_data/demo_HandyCNV.bed")

download.file("https://raw.githubusercontent.com/JH-Zhou/HandyCNV/master/vignettes/Demo_data/demo_HandyCNV.fam",destfile = "Demo_data/demo_HandyCNV.fam")

download.file("https://raw.githubusercontent.com/JH-Zhou/HandyCNV/master/vignettes/Demo_data/demo_HandyCNV.bim",destfile = "Demo_data/demo_HandyCNV.bim")
...
The 'get_haplotype' function can help us to get the haplotype for all samples by given region. But now the application is limited, it only accept the phased genotype by Beagle (Version 5.1) with physical position, we may expend the compatibility later if necessary. The standard format of phased genotype and demo data used at here can be download from our github repository (https://github.com/JH-Zhou/HandyCNV/tree/master/vignettes/OARS_chr6). Now, let's go ahead and make haplotypes. 

Extracting the haplotype will take three steps and combine three functions. The first step is to prepare the standard format for 'HandyCNV'. We can use 'prepare_geno' function to prepare the phased genotype. After running, it will return three list files contain the map, genotype and the columns index file.
```{r, chunk 47}
download.file("https://raw.githubusercontent.com/JH-Zhou/HandyCNV/master/vignettes/OARS_chr6.vcf",destfile = "OARS_chr6.vcf")
genotype <- prep_phased(phased_geno = "OARS_chr6.vcf") #'OARS_chr6.vcf' is the demo data, need to download and save it in your working directory 
```

Second step, we need to assign the interested region. Here we take the *NCAPG* gene as example, it located on BTA6:37,332,024-37,378,129 bp. The idea in this step is to find out the closest SNPs in this SNP chip.
```{r, chunk 48}
region <- closer_snp(phased_input = genotype$map, #the map file in the list generated in 'pre_phased' function above
                     chr = 6, start = 37.332024, end = 37.378129) #physical position unit in 'megabases'
```
...
```{r, chunk 49}
haplotype <- get_haplotype(geno = genotype, pos = region)
```
...
```{r, chunk 50}
data.frame(table(haplotype$hap_type$Recode_Type, dnn = "Haplotype"))
haplotype$hap_index
```
