...
rd
rd <- rd[elementMetadata(rd)[,4]>200]
...
links <- gsub("N","",breaks_variants$ALT)
links <- gsub("\\[","",links)
links <- gsub("\\]","",links)
links_seqname <- sapply(strsplit(links,":"),"[[",1)
links_seqname <- gsub("^","chr",links_seqname)
links_start <- sapply(strsplit(links,":"),"[[",2)
breaks_variants_circos <- data.frame(breaks_variants[,3:5],links_seqname,links_start,links_start)
breaks_variants_circos
...
RCircos.Validate.Genomic.Data(genomic.data = breaks_variants_circos,plot.type = "link",genomic.columns = 3)
#track.num <- 5
#RCircos.Link.Plot(link.data=breaks_variants_circos, track.num = 5,start.pos = 0.75,by.chromosome = TRUE)

data(RCircos.Ribbon.Data)
RCircos.Ribbon.Plot(ribbon.data=breaks_variants_circos,track.num = 5, by.chromosome = FALSE, twist = FALSE)

```

```{r, loadLibraries,  results='hide'}
#loading the required libraries needed for the genome browser
library(igvR)
library(MotifDb)
library(BSgenome)
...
```{r, createLoad}
#creating igvR instance and initializing a websocket between R 
#and the web page over which commands and data will travel to
igv <- igvR()
setBrowserWindowTitle(igv, "Genetic Variation")
setGenome(igv, "mm10")

```

```{r, show region}
new.region <- "chr2:88,882,214-88,884,364"
showGenomicRegion(igv, new.region)
```

```{r bamfile,  results='hide'}
#expandimg the region on display and creating a Granges object for other genomic queries
bamFile <-  "bam/SRX5884275.bam"
file.exists(bamFile)
which <- GRanges(seqnames="chr2", ranges = IRanges(98667057, 98667057))
param <- ScanBamParam(which=which, what = scanBamWhat())
x <- readGAlignments(bamFile, index="bam/SRX5884275.bam", 
                     use.names=FALSE, param=param,with.which_label = FALSE)
track <- GenomicAlignmentTrack("bam/SRX5884275", x, visibilityWindow=30000, trackHeight=100)  # 30000 default
